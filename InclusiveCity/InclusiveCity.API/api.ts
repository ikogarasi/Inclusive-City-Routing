...
export class StructureClient extends ApiBase {
    ...
    getInclusiveInfrastructure(latitude: number | undefined, longitude: number | undefined, around: number | undefined, toilets: boolean | undefined, busStops: boolean | undefined, kerbs: boolean | undefined, tactilePaving: boolean | undefined, ramps: boolean | undefined, shouldRetrieveRating: boolean | undefined, shouldRetrieveReviews: boolean | undefined, shouldGetImages: boolean | undefined): Promise<GetStructuresDto> {
    ...
    protected processGetInclusiveInfrastructure(response: Response): Promise<GetStructuresDto> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
